...
PushNotifications.prototype.send = function (pushId, data, callback) {
	...
	_.extend(gcmOptions, settings.gcm.options);
	_.extend(apnOptions, settings.apn.options, settings.apn);
	_.extend(admOptions, settings.adm.options, settings.adm);
	GCMSender = new gcm.Sender(settings.gcm.id, gcmOptions);
	...
	if (regIdsGCM[0] != undefined) {
		...
		anroidData = _.omit(anroidData,'titleIos');
		messageGCM = new gcm.Message(anroidData);
	...
	if (regIdsAPN[0] != undefined) {
		...
		messageAPN.alert = data.titleIos;
		var dataIos = _.omit(data, 'title','titleIos');
		messageAPN.payload = dataIos;
